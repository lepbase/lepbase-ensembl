...
## BEGIN LEPBASE MODIFICATIONS...
    'mail_user'     => $sd->ENSEMBL_MAIL_USER,
    'mail_pass'     => $sd->ENSEMBL_MAIL_PASS,
    'mail_port'     => $sd->ENSEMBL_MAIL_PORT,
    'mail_use_tls'  => $sd->ENSEMBL_MAIL_USE_TLS,
...
## BEGIN LEPBASE MODIFICATIONS...
	...
	$mailer->auth($self->{'mail_user'},$self->{'mail_pass'});
	$mailer->mail($self->{'from'});
	$mailer->to($self->{'to'});
## ...END LEPBASE MODIFICATIONS

## BEGIN LEPBASE MODIFICATIONS...
	$mailer->data;
	$mailer->datasend("From:".$self->{'from'}."\n");
	$mailer->datasend("To:".$self->{'to'}."\n");
	$mailer->datasend("Subject:".$self->{'subject'}."\n");
	$mailer->datasend("Reply-To:".$self->{'reply'}."\n");
    $mailer->datasend("X-URL:".$self->{'base_url'}."\n");
	$mailer->datasend("Date:$time_string\n");
	$mailer->datasend($self->{'message'});
	$mailer->dataend;
	$mailer->quit;
## ...END LEPBASE MODIFICATIONS
    } catch {
